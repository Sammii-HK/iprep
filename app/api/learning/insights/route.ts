	...
	try {
		...
		if (!isPremiumUser(user)) {
			throw new ValidationError(
				"Premium access required for learning insights"
			);
		}
		...
		try {
			...
			if (
				...
				console.warn(
					"topForgottenPoints column not found, fetching insights without it"
				);
				// Try to fetch using raw query to avoid Prisma type issues
				const rawInsights = await prisma.$queryRawUnsafe<
					Array<{
						id: string;
						userId: string;
