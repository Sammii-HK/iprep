...
export function getConfig(): Config {
	...
	if (isBuildTime || !process.env.DATABASE_URL) {
		config = {
			...
			limits: {
				maxAudioSizeMB:
					parseInt(getOptionalEnvVar("MAX_AUDIO_SIZE_MB"), 10) || 50,
				rateLimitRequests:
					parseInt(getOptionalEnvVar("RATE_LIMIT_REQUESTS"), 10) || 10,
				rateLimitWindowMs:
					parseInt(getOptionalEnvVar("RATE_LIMIT_WINDOW_MS"), 10) || 60000,
			},
	...
	try {
		...
		if (process.env.NODE_ENV === "development") {
			...
			console.warn(
				"Some features may not work until environment variables are set"
			);
